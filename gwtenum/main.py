...
def get_global_val(varname, html_file):
    for html_line in html_file:
        match = re.match(
            ".*," + re.escape(varname) + r"\='([A-Za-z0-9_\.\!\@\#\$\%\^\&\*\(\)"
            r"\-\+\=\:\;\"\|\\\\/\?\>\,\<\~\`]+)',",
            html_line,
        )
...
def main():
    parser = OptionParser(
        usage="usage: %prog [options]", description=desc, version="%prog 0.10"
    )
    ...
    if options.proxy:
        handlers.append(
            urllib.request.ProxyHandler({"http": "http://" + options.proxy})
        )
    ...
    if options.basicauth:
        ...
        basic_auth_encoded = base64.b64encode(f"{username}:{password}".encode()).strip()
        req.add_header("Authorization", f"Basic {basic_auth_encoded}")
    ...
    for html_file in html_files:
        ...
        async_error_mess = ""
        invoke_method = ""
        cache_html = f"{gwt_docroot}{html_file}.cache.html"
        print(f"Analyzing {cache_html}")
        ...
        for line in the_page:
            ...
            rpc_method_match = re.match(
                r"^function \w+\(.*method\:([A-Za-z0-9_\$]+),.*$", line
            )
            ...
            if rpc_method_match:
                ...
                methods.append(f"{method_name}( ".replace("_Proxy.", "."))
                ...
                for i in range(0, len(rpc_js_function)):
                    ...
                    if try_match:
                        i += 1
                        func_match = re.match(
                            r"^([A-Za-z0-9_\$]+)\(.*", rpc_js_function[i]
                        )
                        payload_function = ""
                        if func_match:
                        ...
                        if param_match:
                            num_of_params = int(
                                get_global_val(param_match.group(1), the_page)
                            )
                        ...
                        for j in range(0, num_of_params):
                            ...
                            if param_var_match:
                                param = get_global_val(
                                    param_var_match.group(1), the_page
                                )
    ...
    line_decor = "\n===========================\n"
    print(f"\n{line_decor}Enumerated Methods{line_decor}")
